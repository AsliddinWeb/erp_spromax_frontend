...
import { Search, Plus, Edit, Eye, EyeOff, AlertTriangle, Trash2 } from 'lucide-vue-next'
...
async function handleDelete(row) {
  const ok = await confirm({
    ...
    message: `"${row.name}" ehtiyot qismni o'chirmoqchimisiz?`,
    confirmText: 'O\'chirish',
    variant: 'danger',
  })
  if (!ok) return
  try {
    await maintenanceApi.deleteSparePart(row.id)
