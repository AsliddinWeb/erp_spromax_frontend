<template>
  <div class="space-y-6">
    ...
    <ScrapStockView v-else-if="activeTab === 'scrap'" />
  </div>
</template>

<script setup>
import { ref, computed } from 'vue'
import { GitBranch, Cog, Package, Clock, Recycle } from 'lucide-vue-next'
...
import ScrapStockView from './ScrapStockView.vue'
...
const tabs = [
  ...
  { key: 'scrap',    label: 'Atxot sklad',        icon: Recycle,  roles: ['superadmin','admin','director','production_manager','operator'] },
]

const visibleTabs = computed(() => tabs.filter(t => hasRole(t.roles)))
</script>