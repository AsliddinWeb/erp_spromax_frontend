  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          Moliyaviy holat va tranzaksiyalar
        </p>
      </div>
      <button
        @click="load"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
      ...
      <div class="xl:col-span-2 bg-white dark:bg-dark-800 rounded-2xl border border-gray-100 dark:border-dark-border p-5">
        <h3 class="font-semibold text-gray-900 dark:text-white mb-5">Moliyaviy holat</h3>
        ...
        <div class="grid grid-cols-2 gap-3 mt-5 pt-5 border-t border-gray-100 dark:border-dark-border">
          <div class="bg-gray-50 dark:bg-dark-700 rounded-xl p-3">
            ...
            <div class="font-bold text-success">{{ formatMoney(data?.income_today) }}</div>
          </div>
          <div class="bg-gray-50 dark:bg-dark-700 rounded-xl p-3">
            <div class="text-xs text-gray-400 mb-1">Bugungi xarajat</div>
            <div class="font-bold text-danger">{{ formatMoney(data?.expense_today) }}</div>
          </div>
          <div class="bg-gray-50 dark:bg-dark-700 rounded-xl p-3">
            <div class="text-xs text-gray-400 mb-1">Shu oy daromad</div>
            <div class="font-bold text-success">{{ formatMoney(data?.income_this_month) }}</div>
          </div>
          <div class="bg-gray-50 dark:bg-dark-700 rounded-xl p-3">
            <div class="text-xs text-gray-400 mb-1">Shu oy xarajat</div>
            <div class="font-bold text-danger">{{ formatMoney(data?.expense_this_month) }}</div>
          </div>
        </div>
      </div>
...
import { RefreshCw, ChevronRight, Banknote, TrendingUp, DollarSign, BarChart3, Plus, FileText } from 'lucide-vue-next'
...
const quickLinks = [
  { path: '/finance', label: 'Yangi tranzaksiya', desc: 'Kirim/chiqim qo\'shish', icon: Plus,      bg: 'bg-primary/10', color: 'text-primary' },
  { path: '/finance', label: 'Tranzaksiyalar',    desc: 'Barcha tranzaksiyalar', icon: Banknote,  bg: 'bg-success/10', color: 'text-success' },
  { path: '/finance', label: 'P&L hisobot',       desc: 'Foyda va zarar',        icon: TrendingUp, bg: 'bg-warning/10', color: 'text-warning' },
  { path: '/finance', label: 'Cash Flow',         desc: 'Pul oqimi hisobot',     icon: FileText,  bg: 'bg-danger/10',  color: 'text-danger'  },
]
...
async function load() {
  loading.value = true
  try {
    const { data: res } = await financeApi.getStatistics()
    data.value = res
  } catch {
